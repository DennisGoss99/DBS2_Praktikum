...
CREATE TABLE IF NOT EXISTS `WaffleDB`.`ProductOrder` (
  ...
  CONSTRAINT `fk_ProductOrder_Order1`
    FOREIGN KEY (`idOrder`)
    REFERENCES `WaffleDB`.`WaffleOrder` (`idOrder`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION,
...
-- -----------------------------------------------------
-- Table `WaffleDB`.`WaffleOrder`
-- -----------------------------------------------------
DROP TABLE IF EXISTS `WaffleDB`.`WaffleOrder` ;

CREATE TABLE IF NOT EXISTS `WaffleDB`.`WaffleOrder` (
  `idOrder` INT NOT NULL,
  `idStore` INT NOT NULL,
...
CREATE INDEX `fk_Order_Store1_idx` ON `WaffleDB`.`WaffleOrder` (`idStore` ASC) VISIBLE;
